...
def get_camera_params(resolution= 512, fov=45, elev_angle=-20, azim_angle=0):
    ...
    normal_rotate =  util.rotate_y(-azim)[:3, :3] @ util.rotate_x(-elev)[:3, :3]
    mvp    = proj_mtx @ mv
...
class DatasetItem3D(torch.utils.data.Dataset):
    ...
    def _validate_scene(self, itr):
        ...
        prompt_index = 0
        mv     = util.translate(0, 0, -3) @ (util.rotate_x(rotate_x) @ util.rotate_y( ang ))
        normal_rotate =  util.rotate_y(0)[:3, :3]
        mvp    = proj_mtx @ mv
        campos = torch.linalg.inv(mv)[:3, 3]
        return mv[None, ...], mvp[None, ...], campos[None, ...], self.FLAGS.display_res, self.FLAGS.spp, normal_rotate[None,...], prompt_index
    ...
    def _train_scene(self, itr):
        ...
        if ((itr+1)/self.FLAGS.batch) <=self.FLAGS.coarse_iter:
            ...
            normal_rotate =  util.rotate_y(rotate_y1)[:3, :3] @ util.rotate_x(rotate_x1)[:3, :3]
        else:
            normal_rotate =  util.rotate_y(0)[:3, :3]@util.rotate_x(0)[:3, :3]
        mvp    = proj_mtx @ mv
        campos = torch.linalg.inv(mv)[:3, 3]
        return mv[None, ...], mvp[None, ...], campos[None, ...], self.FLAGS.display_res, self.FLAGS.spp, normal_rotate[None,...], prompt_index
    ...
    def __len__(self):
        if self.gif == True:
            ...
            return 16 if self.validate else (self.FLAGS.iter + 1) * self.FLAGS.batch
